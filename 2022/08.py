...
def tree_visible_and_scene(grid, x, y):
    ...
    scenicity = np.prod([next((i+1 for i, tree in enumerate(row)
                        if tree >= height), row.size) for row in [u, d, l, r]])

    return visibility, scenicity


input = lines()
grid = np.array([[*line[:-1]] for line in input], int)
vis_grid = grid.copy()
